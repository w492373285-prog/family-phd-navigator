<template>
  <main class="app-shell">
    ...
    <section v-if="activeTab === 'schools'" class="screen">
      <PageHeader title="学校 / 导师筛选" subtitle="先用国家TOP3缩小范围，再按专业关键词和导师经费做精筛。" />
      <EmptyState v-if="!plan" @go="activeTab = 'form'" />
      <template v-else>
        <article v-for="school in plan.schools" :key="school.id" class="panel">
          <h2>{{ school.name }}</h2>
        ...
        <article v-for="mentor in plan.mentors" :key="mentor.id" class="mentor-row">
          <UserRound :size="18" />
          <div>
            <strong>{{ mentor.name }}</strong>
            <p>{{ mentor.school }} · {{ mentor.research_area }}</p>
            <small>{{ mentor.fit_notes }}</small>
          </div>
        </article>
      </template>
    </section>
